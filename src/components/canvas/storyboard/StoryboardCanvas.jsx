...
/* ─────────────────────────────────────────────
   Inner component — rendered INSIDE ReactFlowProvider so it can use
   useReactFlow() (needed to convert a click's screen position into
   flow/canvas coordinates for placing a new pinned comment, and to
   pan/zoom to a pin when its scene card's comment badge is clicked).
───────────────────────────────────────────── */
function StoryboardCanvasInner({
  storyboard,
  viewMode,
  ...
  const openCommentsForNode = useCallback((nodeId) => {
    const pin = comments.find((c) => c.node_id === nodeId && !c.resolved);
    if (!pin) return;
    setFocusedNodeId(nodeId);
    setCenter(pin.pin_x, pin.pin_y, { zoom: 1, duration: 400 });
  }, [comments, setCenter]);
  ...
  const builtNodes = useMemo(() => {
    ...
    if (viewMode === "freeform") {
      ...
      sceneRfNodes = sceneNodes.map((n) => ({
        ...
        data: {
          ...n.data,
          onJumpToScene,
          onOpenComments: () => openCommentsForNode(n.id),
          commentCount: commentCountByNodeId[n.id] || 0,
        },
      }));
    } else {
    ...
    const commentRfNodes = comments.map((c) => ({
      ...
      data: {
        ...
        onResolve: onResolveComment,
        onDelete: onDeleteComment,
        forceExpanded: c.node_id === focusedNodeId,
      },
    }));
    ...
    storyboard, viewMode, editable, onJumpToScene, commentCountByNodeId,
    handleFrameLabelChange, comments, currentUserId, canManageComment,
    onResolveComment, onDeleteComment, focusedNodeId, openCommentsForNode,
  ]);
