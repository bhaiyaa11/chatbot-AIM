...
//             <option value="MARK_AMERICAN_MALE">🇺🇸 Mark </option>,
//             <option value="KAIRA_AMERICAN_FEMALE">🇺🇸 Kaira</option>
...
<div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"7px 12px", borderBottom:"1px solid rgba(255,255,255,.06)", background:"#111", gap:"6px", flexWrap:"wrap" }}>
  <div style={{ display:"flex", gap:"6px", flexWrap:"wrap", alignItems:"center" }}>
    ...
    {[["british","🇬🇧 British"],["american","🇺🇸 American"],["australian","🇦🇺 Australian"]].map(([val, label]) => (
      <button key={val} onClick={() => setAccent(val)}
        style={{
          background: accent === val ? "rgba(168,85,247,.1)" : "none",
          border: `1px solid ${accent === val ? "rgba(168,85,247,.5)" : "rgba(255,255,255,.07)"}`,
          borderRadius:"9999px",
          color: accent === val ? "rgba(200,160,255,.9)" : "rgba(255,255,255,.5)",
          cursor:"pointer", fontSize:"11px", fontFamily:"'Inter',sans-serif", padding:"4px 10px"
        }}>
        {label}
      </button>
    ))}
    ...
    {[["conversational","Conversational"],["advertising","Advertising"],["social_media","Social Media"]].map(([val, label]) => (
      <button key={val} onClick={() => setTone(val)}
        style={{
          background: tone === val ? "rgba(168,85,247,.1)" : "none",
          border: `1px solid ${tone === val ? "rgba(168,85,247,.5)" : "rgba(255,255,255,.07)"}`,
          borderRadius:"9999px",
          color: tone === val ? "rgba(200,160,255,.9)" : "rgba(255,255,255,.5)",
          cursor:"pointer", fontSize:"11px", fontFamily:"'Inter',sans-serif", padding:"4px 10px"
        }}>
        {label}
      </button>
    ...
    {/* ── Voice Select (filtered) ── */}
    <select
      value={selectedVoice}
      onChange={(e) => setSelectedVoice(e.target.value)}
      style={{ background:"#111", border:"1px solid rgba(255,255,255,.08)", borderRadius:"9999px", color:"rgba(255,255,255,.75)", fontSize:"11px", fontFamily:"'Inter',sans-serif", padding:"4px 12px", cursor:"pointer", outline:"none" }}>
      {VOICES
        .filter(v => v.accent === accent && v.tone.includes(tone))
    ...
    <button
      ...
      style={{ background:"none", border:"none", color:"rgba(255,255,255,.25)", cursor:"pointer", fontSize:"11px", fontFamily:"'Inter',sans-serif", padding:"4px 6px", borderRadius:"9999px" }}
      title="Reset filters">
      ↺ Reset
    </button>
