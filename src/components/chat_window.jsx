...
//   const { activeChat, addMessage, updateLastMessage } = useChat();
//   const chatId = activeChat?.id ?? null;
//   // isEmpty: show landing page when no local messages loaded and not currently fetching
...
//         `${API_BASE_URL}/messages?chat_id=${chatIdParam}&page=${pageNum}&limit=20`
//       );
//       const data = await res.json();
...
//       // Backend returns newest-first; reverse for chat order (old → new, top → bottom)
//       const ordered = [...fetched].reverse().map(m => ({
...
//     // ── Append user message to local state for rendering ──
//     setMessages(prev => [...prev, { sender: "user", text: input, content: input, prompt: "", files: filePreviewData }]);
//     setResearchData(null);
//     setInput("");
//     setFiles([]);
//     const formData = new FormData();
...
//       // ── Append placeholder assistant message to local state ──
//       setMessages(prev => [...prev, { sender: "bot", text: "", content: "", prompt: "", files: [] }]);
...
//       setResearchId(null);
//       updateLastMessage(chatId, fullText, finalPrompt);
//       // ── Also update local state with final content ──
...
//       console.error("generateScript error:", err);
//       safeAddMessage({ role: "assistant", content: "⚠️ Server error" });
//       setMessages(prev => [...prev, { sender: "bot", text: "⚠️ Server error", content: "⚠️ Server error", prompt: "", files: [] }]);
//     }
//   };
...
//       // Backend returns newest-first; reverse for chat order (old → new, top → bottom)
//       const ordered = [...fetched].reverse().map(m => ({
//         // FIX 2: role is now returned from backend, so this mapping works correctly
//         sender: m.role === "assistant" ? "bot" : "user",
//         text: m.content,
//         content: m.content,
//         // FIX 3: prompt is now returned from backend
//         prompt: m.prompt ?? "",
//         files: [],
//       }));
