...
function ChatWindow() {
  ...
  // ── Floating menu — handle action (Improve / Regenerate / Shorten) ──
  const handleFloatingAction = async (instruction) => {
    ...
    await safeAddMessage({ role: "user", content: prompt });
  ...
  // ── Floating menu — handle Ask AI (custom prompt) ──────────────
  const handleAskAI = async (customPrompt) => {
    if (!selectedText) return;
    setMenuPosition(null);

    const prompt = `${customPrompt}:\n\n${selectedText}`;
    await handleFloatingAction(prompt);
  };
    ...
    <div className="chat-window" onMouseUp={handleMouseUp}>
      ...
      {isEmpty ? (
        ...
        <div className="chat-container">
          ...
          {/* ── Floating Edit Menu — now wired up ── */}
          <FloatingEditMenu
            position={menuPosition}
